...
(() => {
  ...
  const COLOR = {
    ...
    OUTER: "#333332",
    INNER: "#1B3664",
    BUMPER: "#561273", //"#DB2B30",
    BUMPER_LIT: "#fff3bf",
    BUMPER_ACTIVE: "#ad1308",
    PADDLE: "#F0F4F3",
    PINBALL: "#FFB900"
  };
  const GRAVITY = 0.75;
  ...
  const PADDLE_PULL = 0.0025;
  const MAX_VELOCITY = 50;

  // score elements
  const $currentScore = $(".current-score span");
  const $highScore = $(".high-score span");
  const hitSound = new Sound("https://www.w3schools.com/graphics/bounce.mp3");
  const newBumpers = [[165, 340], [285, 340], [225, 405], [225, 75]];
  ...
  function createStaticBodies() {
    Matter.World.add(world, [
      ...
      wall(90, 410, 15, 100, COLOR.INNER, -0.7), //długa
      wall(110, 360, 15, 60, COLOR.INNER, -0.7),

      // top bumpers (left, mid, right)
      circleBumper(105, 250),
      circleBumper(345, 250),
      circleBumper(225, 250),
    ...
    flap = world.bodies.find(el => el.label === "flap-flap");
    flap.render.fillStyle = COLOR.BUMPER_ACTIVE;
  }

  function flapflap(x, y) {
    // eslint-disable-next-line
    const flapflap = Matter.Bodies.circle(x, y, 15, {
      label: "flap-flap",
      isStatic: true,
      render: {
    ...
    flapflap.restitution = BUMPER_BOUNCE;
    return flapflap;
  }
  ...
  // eslint-disable-next-line
  function pingBumper(_bumper) {
    updateScore(currentScore + 10);
    hitSound.play();
    // flash color
    // eslint-disable-next-line
    _bumper.render.fillStyle = COLOR.BUMPER_LIT;
    // console.log(Composite.allBodies(world));
    setTimeout(() => {
      // eslint-disable-next-line
      _bumper.render.fillStyle = COLOR.BUMPER;
    }, 100);

    if (addedBumbers < newBumpers.length && minimumScore <= currentScore) {
      setTimeout(() => {
        Matter.World.add(
          world,
          circleBumper(newBumpers[addedBumbers][0], newBumpers[addedBumbers][1])
        );
        addedBumbers += 1;
        minimumScore += 30;
      }, 1500);
  ...
  function createEvents() {
    ...
    Matter.Events.on(engine, "beforeUpdate", () => {
      ...
      if (
        pinball.position.x > 450 &&
        pinball.position.y > 755 &&
        pinball.velocity.y > 0
      ) {
        Matter.Body.setVelocity(pinball, { x: 0, y: -50 });
      }
    });
    ...
    $(".start-trigger").on("mousedown touchstart", () => {
      launchPinball();
    });
  }

  function removePinball() {
    if (!pinball) {
      return;
    }
    Matter.Composite.remove(world, pinball);
  }
  ...
  function bumpers() {
    const leftBumper = Matter.Bodies.rectangle(130, 585, 70, 15, {
      isStatic: true,
      angle: 3.75,
      chamfer: { radius: 2 },
      render: { fillStyle: COLOR.INNER }
    });
    const leftLaunchPad = Matter.Bodies.rectangle(135, 580, 65, 10, {
      label: "bumper",
      isStatic: true,
      angle: 3.75,
      chamfer: { radius: 2 },
    ...
    leftLaunchPad.restitution = 5;

    const rightBumper = Matter.Bodies.rectangle(318, 585, 70, 15, {
      isStatic: true,
      angle: 5.65,
      chamfer: { radius: 2 },
      render: { fillStyle: COLOR.INNER }
    });
    const rightLauchPad = Matter.Bodies.rectangle(315, 580, 65, 10, {
      label: "bumper",
      isStatic: true,
      angle: 5.65,
      chamfer: { radius: 2 },
    ...
    rightLauchPad.restitution = 5;
    //
    const smallBumper = Matter.Bodies.rectangle(130, 455, 0, 0, {
      isStatic: true,
      angle: 8.45,
      chamfer: { radius: 2 },
      render: { fillStyle: COLOR.INNER }
    });
    const smallLauchPad = Matter.Bodies.rectangle(135, 438, 30, 10, {
      label: "bumper",
      isStatic: true,
      angle: 8.8,
      chamfer: { radius: 2 },
  ...
  function circleBumper(x, y) {
    // eslint-disable-next-line
    const bumper = Matter.Bodies.circle(x, y, 15, {
      label: "bumper",
      isStatic: true,
      render: {
        fillStyle: COLOR.BUMPER
